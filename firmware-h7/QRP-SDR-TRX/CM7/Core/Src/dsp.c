...
void nbfm_demod(float *I_in, float *Q_in, float *audio_out, int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        /* DIAGNOSTIC: agc_process() bypassed. With no/floating RF input,
           the discriminator output sits near zero almost constantly (below
           agc_process()'s 0.1 threshold), so its gain continuously ramps
           toward its 10x ceiling during that silence - then any small
           noise blip gets amplified up to 10x, clipping hard against the
           DAC's [0,4095] bound in rx_chain.c. That's the distortion being
           tested here. (void) keeps agc_fm's state ticking over so it's
           not stuck if this gets re-enabled later, without using its
           output. */
        (void)agc_process(deemph_out, &agc_fm);

        audio_out[i] = deemph_out;
    }
}
