...
#include "stm32h7xx.h"      /* must precede arm_math.h: defines __FPU_PRESENT */
#include "arm_math.h"
...
static int rearm_samples;
static int has_armed_once;
static volatile uint32_t underrun_count;

/* Diagnostic: is freedv_nin() jittering symmetrically around its nominal
   value (normal timing-tracker behaviour) or is it systematically biased
   one way (which would mean a real ADC/DAC sample-rate mismatch, not just
   tracker noise)? See freedv_chain_nin_stats(). */
static int32_t nin_min = 0x7fffffff;
static int32_t nin_max = 0;
static int64_t nin_sum;
static uint32_t nin_count;

/* Diagnostic: how often freedv_chain_put_audio48()'s input_is_48k (2400B)
   path finds in_fifo too full to accept a chunk - those samples are
   silently dropped (codec2_fifo_write() just isn't called), not queued or
   reported anywhere else. See freedv_chain_in_drops(). */
static volatile uint32_t in_fifo_drops;
...
int freedv_chain_init(int chain_mode)
{
    ...
    /*
     * Steady-state recovery threshold, used instead of prefill_samples once
     * playback has armed at least once - see freedv_chain_get_speech48().
     * One frame's worth is enough to avoid immediately underrunning again;
     * demanding the full 3-frame prefill_samples cushion on every recovery
     * (as this used to) turned a single missed 700D frame (measured live on
     * hardware: sync holding, one dropped frame roughly every 2s on a
     * genuinely marginal-SNR signal - not a bug, just real HF/analog
     * channel loss) into ~250ms+ of silence before anything played again,
     * which is what made audio come through "only seldom" rather than with
     * occasional brief dropouts.
     *
     * Two frames, not one: measured live on hardware (DWT cycle counter),
     * process_block() costs a steady 28% of its 40ms budget for 1600 -
     * timing/CPU load isn't the issue. The real cause is that the
     * decimation path below pushes exactly one frame's worth of 8kHz
     * samples per block (2x DECIM_OUT_CHUNK) into in_fifo, and
     * freedv_nin() - the modem's own timing tracker - legitimately asks
     * for a bit more than that some cycles as it dithers around the
     * nominal rate. With only one frame's cushion, any such cycle finds
     * out_fifo empty and registers a fresh underrun even at good SNR
     * (measured: 33 underruns in ~10s at 4dB SNR, well above where 1600
     * should struggle) - there was no standing reserve to absorb ordinary
     * timing-tracker jitter, only enough to recover after already running
     * dry. A two-frame reserve lets one lean cycle draw down the spare
     * frame instead of immediately starving playback.
     */
    rearm_samples = 2 * freedv_get_n_speech_samples(fdv);
    if (rearm_samples > OUT_FIFO_LEN / 2)
        rearm_samples = OUT_FIFO_LEN / 2;
    has_armed_once = 0;
 ...
 *
 * Raised from 1 to 2 after measuring 1600's actual process_block() cost on
 * hardware (DWT cycle counter): a steady 28% of its 40ms budget, not the
 * unknown-but-assumed-tight figure this comment's original reasoning was
 * hedging against. freedv_nin() (the modem's timing tracker) legitimately
 * asks for more than the nominal ~320 samples/frame on some cycles, which
 * a cap of 1 could only ever catch up from one frame at a time even with
 * backlog waiting - still measurably underrunning the playback jitter
 * buffer (rearm_samples below) even at good SNR. Two attempts per call, at
 * this CPU cost, still leaves enormous margin against the block deadline.
 */
#define MAX_FRAMES_PER_CALL 2
...
static void run_demod(void)
{
    ...
    for (int frame = 0; frame < MAX_FRAMES_PER_CALL; frame++)
    {
        ...
        if (nin < nin_min) nin_min = nin;
        if (nin > nin_max) nin_max = nin;
        nin_sum += nin;
        nin_count++;
...
void freedv_chain_put_audio48(const float *audio48, int n)
{
    ...
    if (input_is_48k)
    {
        ...
        /* Twice, not once: the decimation path below calls run_demod() once
           per DECIM_IN_CHUNK (960 samples), so a 1920-sample block gets two
           independent attempts to drain in_fifo. This path pushed the same
           1920 samples in one go and called run_demod() only once, so any
           call where freedv_nin() came in under what was pushed (normal
           timing-tracker jitter, not an error) had no second chance to
           catch up within the same block - the shortfall could only carry
           over to the next call, one attempt at a time. Matches the other
           path's cadence instead of leaving 2400B alone at a tighter
           margin than 1600 for no reason tied to the mode itself. */
        run_demod();
        run_demod();
        return;
    }
...
int freedv_chain_get_speech48(float *audio48, int n)
{
    ...
    /* Let a cushion bank up before playing anything; see prefill_samples/
       rearm_samples. Cold start (has_armed_once == 0) waits for the full
       cushion; recovering from a later underrun only needs one frame's
       worth, so a single dropped frame costs one frame of silence, not a
       full cushion rebuild. */
    if (!playback_armed)
    {
        int threshold = has_armed_once ? rearm_samples : prefill_samples;

        if (out_stage_used + codec2_fifo_used(out_fifo) < threshold)
        {
            memset(audio48, 0, n * sizeof(float));
            return 0;
        }
...
unsigned freedv_chain_underruns(void) { return underrun_count; }

void freedv_chain_nin_stats(int32_t *min, int32_t *max, int32_t *avg)
{
    *min = (nin_count > 0) ? nin_min : 0;
    *max = nin_max;
    *avg = (nin_count > 0) ? (int32_t)(nin_sum / (int64_t)nin_count) : 0;
}

unsigned freedv_chain_in_drops(void) { return in_fifo_drops; }
