...
void HAL_ADC_MspInit(ADC_HandleTypeDef* hadc)
{
  ...
  if(hadc->Instance==ADC1)
  {
    ...
    /**ADC1 GPIO Configuration -- RX IF in
    PC0     ------> ADC1_IN10
    */
    ...
    /* Halfword: the ADC is 12-bit, so words wasted half the DMA buffer. The
       buffer doubles as the input FIFO for the buffered modes, so its depth
       matters. NOTE: regenerating from the .ioc will revert this. */
  ...
  else if(hadc->Instance==ADC2)
  {
    ...
    /**ADC2 GPIO Configuration -- TX mic in
    PA3     ------> ADC2_IN3
    */
...
void HAL_DAC_MspInit(DAC_HandleTypeDef* hdac)
{
  ...
  if(hdac->Instance==DAC)
  {
    ...
    PA4     ------> DAC_OUT1  (RX audio out)
    PA5     ------> DAC_OUT2  (TX IF out)
    */
